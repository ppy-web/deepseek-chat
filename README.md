...
当前 Vite `base` 和路由 `base` 均为 `/deepseek-chat/`，对应访问地址为：

```text
https://ppy-web.github.io/deepseek-chat/
```

如果推送到 `gh-pages` 后访问 404，请先在 GitHub 仓库 `Settings -> Pages` 中确认发布源为 `Deploy from a branch`，分支选择 `gh-pages`，目录选择 `/ (root)`。如果部署到其他路径，需要同步调整 `vite.config.ts` 和 `src/router.ts` 中的基础路径。
